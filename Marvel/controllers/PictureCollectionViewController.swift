...
class PictureCollectionViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        if enableCloseButton {
            ...
            closeButton.setBackgroundImage(UIImage(named: "icn-nav-close-white"), for: UIControl.State.normal)
        ...
        if let image = self.cache.get(hash: hashValue) {
            ...
            Utilities.queues.asyncQueue.addOperation({
                if let localImage = ImageUtilities.getImage(path: storagePath){
                    ...
                    ApiManager.request.getComic(url: url as URL, completionHandler: { (ok: Bool, objects: [Comic]?, error: Error?) in
                        if let comic = objects?.first{
                            API().downloadImage(url: comic.thumbnailURI as URL, storageFilePaths: storagePath, completionHandler: { (image) in
                                if image != nil{
